...
page__container.insertAdjacentHTML("beforebegin", template);
...
function calcDivision(a, b) {
  const result = a / b;
  if (!isFinite(result)) {
    return `Результат ділення: Ділення  не коректне;`;
  } else {
...
funcResult();
funcResult(calcDivision(23, ""));
funcResult(calcDivision(12, 0));
funcResult(calcDivision(12, "12"));
funcResult(calcDivision(12, true));
funcResult(calcDivision(12, false));
funcResult(calcDivision(45, "0"));
funcResult(calcDivision("9", 3));
funcResult(calcDivision(Infinity, -Infinity));
funcResult(calcDivision(Infinity, 1));
funcResult(calcDivision(1, Infinity));
funcResult(calcDivision("work", 100));
